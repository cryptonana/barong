  ...
  describe 'GET /api/v2/admin/activities' do
    ...
    context 'admin user' do
      ...
      context 'admin activities' do
        let!(:first_activity) { create(:activity, topic: 'session', action: 'login', user: first_user, category: 'admin', target_uuid: second_user.uid) }
        let!(:second_activity) { create(:activity, topic: 'session', action: 'login', user: second_user, category: 'admin') }
        let!(:third_activity) { create(:activity, topic: 'otp', action: 'otp::enable', user: second_user, category: 'admin', target_uuid: second_user.uid) }
        it 'returns list of activities' do
        ...
        it 'returns list of activities filtered by affected user' do
          get '/api/v2/admin/activities/admin', headers: auth_header, params: { target_uuid: second_user.uid }
          activities = JSON.parse(response.body)
          expect(activities.count).to eq 2
        end
      end

    ...
    context 'records failed activity if path matches with AUDIT permission path' do
      context 'creates only one thread' do
        ...
        it 'acts as expected' do
          ...
          threads_after = Thread.list.count
          expect(threads_after).to eq(threads_before + 1)

          ## do audit calls
          10.times { delete '/api/v2/auth/api/v2/admin/markets', headers: { 'HTTP_USER_AGENT' => 'random-browser' } }
          sleep 0.01
          expect(Activity.where(category: 'admin').count).to eq(11)
          threads_after = Thread.list.count
          expect(threads_after).to eq(threads_before + 1)
        end
      end
      ...
      context 'without topic specified and without user_uid in params' do
        ...
        it 'for account role' do
          ...
          expect(Activity.last.result).to eq('failed')
        end
    ...
    context 'records succesfull activity if path matches with AUDIT permission path' do
      context 'with different params combination' do
        ...
        context 'for different roles with user_uid in params' do
          it 'create activity and save target_uuid for accountant if user with this uid exists' do
            do_create_session_request_acc
            expect(Activity.where(category: 'admin').count).to eq(0)
            ...
            expect(Activity.last.topic).to eq('markets')
            expect(Activity.last.result).to eq('succeed')
            expect(Activity.last.target_uuid).to eq(user.uid)
          end

          it 'doesnt create activity and save target_uuid for accountant cause user with this uid doesnt exist' do
            do_create_session_request_acc
            expect(Activity.where(category: 'admin').count).to eq(0)
